...
def check_context(self, command_contexts: str):
    if self.context in command_contexts:
        return True
    print(
        "Wrong context! This command must be run from the following context(s): "
        + ", ".join(command_contexts)
    )
    return False
...
def update_prompt(self):
    ...
    if self.payload:
        ...
        self.prompt = "(" + self.agent.uuid + ") > "
    else:
        self.prompt = (self.context + " > ") if self.context else "> "


def last_seen_str(self, lastseen: float):
    """
...
def agent_info(self, agent: Agent):
    """
...
def add_listener(self, host: str, port: int, secret: str):
    """
    ...
    try:
        ...
        print("Couldnt locate certificate file! Did you run the installer?")
        return
...
def close_listeners(self):
    ...
    """
    if self.listeners:
        print("Executing graceful shutdown of all active listeners...")
        for listener in self.listeners:
